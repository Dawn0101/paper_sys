...
- 2️⃣ student ：console ❌ || overview ❌
- 3️⃣ college_admin ：console ❌ || overview ❌
- 4️⃣ university_admin ：console ❌ || overview ❌

---

## 📥 Git Pull Request 合并步骤

### 1. 切换到主分支（确保你在 main 分支）
```bash
git checkout main
```

### 2. 拉取远程最新代码（包含别人刚合并的 PR 内容）
```bash
git pull origin main
```

### 3. 如果提示冲突（例如：`CONFLICT in views.py`）：
* 用编辑器打开冲突文件
* 找到冲突标记：
...
💡 提示：若第 2 步无冲突，可跳过步骤 3–5，直接执行 `git push`。
